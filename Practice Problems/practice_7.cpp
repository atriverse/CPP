...
public: 
    ...
    /// Overloads the less-than operator (<) to compare two Message objects based on their unique IDs.
    /// This allows Message objects to be sorted using standard algorithms like std::sort().
...
private:
    /// Sorts the messages_ vector in ascending order based on the unique message IDs.
    /// This ensures the messages are printed in the order they were originally sent,
    /// even if they were received in a different order due to network delays.
    void fix_order() {
